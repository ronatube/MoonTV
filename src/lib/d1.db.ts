...
export class D1Storage implements IStorage {
  ...
  async deleteAllPlayRecords(userName: string): Promise<void> {
    try {
      ...
      console.error('Failed to delete play records:', err);
    }
  }
  ...
  async clearAllData(): Promise<void> {
    try {
      const db = await this.getDatabase();
      const result = await db.prepare('SELECT username FROM users').all<any>();
      for (const row of result.results) {
        await db.prepare('DELETE FROM play_records WHERE username = ?').bind(row.username).run();
        await db.prepare('DELETE FROM favorites WHERE username = ?').bind(row.username).run();
        await db.prepare('DELETE FROM skip_configs WHERE username = ?').bind(row.username).run();
        await db.prepare('DELETE FROM search_history WHERE username = ?').bind(row.username).run();
        await db.prepare('DELETE FROM users WHERE username = ?').bind(row.username).run();
      }
